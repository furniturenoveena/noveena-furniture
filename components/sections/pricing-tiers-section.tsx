...
export default function PricingTiersSection() {
  ...
  const pricingTiers = [
    {
      ...
      description:
        "Secure your furniture with a 30% advance payment, with the balance due upon delivery.",
      features: [
    ...
    },
    {
      ...
      description:
        "Complete your purchase with a full payment and enjoy VIP delivery scheduling and exclusive benefits.",
      features: [
    ...
    },
    {
      ...
      description:
        "Perfect for projects and businesses - unlock special bulk pricing with significant savings on multiple items.",
      features: [
  ...
  const renderPricingCard = (tier: PricingTier, index: number) => {
    ...
    const cardHoverClass =
      theme === "dark" ? "hover:bg-card/80" : "hover:bg-white";
    ...
    return (
      <motion.div
        ...
        className={`pricing-tier relative ${cardBgClass} rounded-xl shadow-lg ${
          tier.popular
            ? `shadow-2xl border-2 border-primary/30 z-10 ${
                theme === "dark" ? "shadow-primary/10" : ""
              }`
      ...
      >
        ...
        {tier.badge && (
          <div className="absolute top-2 inset-x-0 flex justify-center">
            <div
              className={`bg-primary px-6 py-2 rounded-full text-sm font-medium shadow-lg transform-gpu group-hover:scale-105 transition-transform duration-300 text-primary-foreground`}
            >
              {tier.badge}
            </div>
          </div>
        ...
        >
          <div
            className={`h-20 w-auto px-4 ${iconBgClass} rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300 shadow-inner`}
          >
            <span
              className={`text-xl font-bold ${
                theme === "dark" ? "text-primary" : tier.iconColor
          ...
          <h3
            className={`text-2xl font-bold mb-3 font-cormorant text-primary${
              tier.popular ? "" : "/90"
          ...
          <ul className="space-y-3 text-left mb-8 flex-grow">
            {tier.features.map((feature, i) => (
              <li key={i} className="flex items-start">
                <div
                  className={`h-6 w-6 rounded-full ${iconBgClass} flex items-center justify-center mr-3 flex-shrink-0`}
                >
                  <Check
