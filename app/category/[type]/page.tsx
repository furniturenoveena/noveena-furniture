...
export default function CategoryPage({ params }: { params: { type: string } }) {
  ...
  useEffect(() => {
    ...
    if (categoryType !== "all") {
      if (categoryType === "imported-used" || categoryType === "brand-new") {
        ...
        const type = categoryType === "imported-used" ? "Imported Used" : "Brand New"
        filteredProducts = filteredProducts.filter((product) => product.type === type)
      } else if (categoryType.includes("imported-used-") || categoryType.includes("brand-new-")) {
        // Handle subcategories
        const [mainType, subCategory] = categoryType.split("-")
        const type = mainType === "imported" ? "Imported Used" : "Brand New"
        const category = subCategory.charAt(0).toUpperCase() + subCategory.slice(1)

        filteredProducts = filteredProducts.filter((product) => product.type === type && product.category === category)
      } else {
        // Handle direct category names
        const category = categoryType.charAt(0).toUpperCase() + categoryType.slice(1)
        filteredProducts = filteredProducts.filter((product) => product.category.toLowerCase() === categoryType)
      }
    }
  ...
  const formatTitle = () => {
    if (categoryType === "all") {
      ...
      return `Imported Used ${category.charAt(0).toUpperCase() + category.slice(1)} Furniture`
    } else if (categoryType.includes("brand-new-")) {
      const category = categoryType.split("-")[2]
      return `Brand New ${category.charAt(0).toUpperCase() + category.slice(1)} Furniture`
    } else {
      return `${categoryType.charAt(0).toUpperCase() + categoryType.slice(1)} Furniture`
    }
  }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handlePriceRangeChange(Number(value), priceRange.max)}
                      >
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handlePriceRangeChange(priceRange.min, Number(value))}
                      >
            ...
            {displayProducts.length === 0 ? (
              ...
              >
                {displayProducts.map((product) => (
                  <ProductCard key={product.id} product={product} variants={fadeInAnimation} />
                ))}
              </motion.div>
            )}
          </div>
